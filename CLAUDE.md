...
- **`main` = `origin/main` = `0b6efc4a7438a6b4edf117897c219038e94bbe65`** (FP-7 code merge; a subsequent docs-only PR advances it further) — **FP-7 is MERGED into `main`** via **PR #13** (merge commit `0b6efc4`; FP-7 checkpoint `15125ec` + cleanup `9b0514c`). Mainline chronology: pre-FP `acae9cb` → **PR #7** merge `be92d7b` (FP-0…FP-4, first FP landing) → **PR #8** merge `8cd8d8b` (docs post-FP-4) → **PR #9** merge `cb1910c` (FP-5) → **PR #10** merge `871ee79` (docs post-FP-5) → **PR #11** merge `fa561b7` (FP-6) → **PR #13** merge `0b6efc4` (FP-7). PR #13 is **MERGED, not pending**.
...
- **ADS "magazine split" investigation — resolved (presentation, not a bug):** the standard tactical reload **intentionally overlaps two magazines** (reserve + main; Infima's own design, replicated). The apparent ADS separation is **framing**: Infima's ADS blends camera FOV **100→78** (plus a positional aiming adjustment); **TacticalMovement currently stays at FOV 100**. The magazine **depletion pose changes only internal follower/round geometry, not magazine body placement** (Persona frame-0-vs-30 diff: body pixel-identical). Placement chain is offset-free (`SnapToTarget`, identity mesh components).
- **Deferred / out of scope:** ADS-framing (FOV 100→78) is the **next proposed slice — still UNAPPROVED**; secondary fidelity = porting the magazine AnimBP/bullet meshes. **No firing, ammo, inventory, or gameplay reload input** implemented.

### 4. NEXT ACTION (resume here)
**FP-0…FP-7 are complete, verified, and MERGED into `main`** (FP-0…FP-4 via PR #7 `be92d7b`; docs via PR #8 `8cd8d8b`; FP-5 via PR #9 `cb1910c`; docs via PR #10 `871ee79`; FP-6 via PR #11 `fa561b7`; **FP-7 via PR #13 `0b6efc4`**). Working tree is clean, `main` is in sync with `origin/main` at `0b6efc4` (before this docs update), and **no FP work is in flight**.
**Next proposed slice: ADS framing (FOV 100→78) — NOT yet approved.** This is the FP-7 investigation's identified follow-up (presentation-only ADS FOV blend gated on `ECombatReadinessState::ADS`; FOV-only first, positional offset a separately-gated fallback). The permanent `PlayFirstPersonReload` seam also awaits future wiring to production reload gameplay. **Do not** begin either, or any firing/ammo/inventory/gameplay-reload work, without an approved slice. Same gated pattern: read-only preflight → propose scope → await explicit approval → implement on a new `feature/*` branch → verify → commit gate → push gate. Re-verify live git + PR state (`git`, `gh pr view`) first.
