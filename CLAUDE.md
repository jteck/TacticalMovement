...
- **Paused implementation state (minimal):** `ABP_TacticalRifle` has **one safe variable added — `PosturePitch : float`** (smoothed readiness posture lowering, degrees). ABP **compiles**. **`PitchN` / `AO_Rifle` NOT modified.** EventGraph posture logic **NOT added yet**. AnimGraph Modify Bone **NOT added yet**. No PIE validation of `PosturePitch` yet.
- **PENDING MANUAL USER STEP (in-editor EventGraph, MCP can't do it cleanly):** in `ABP_TacticalRifle` Event Graph, read `CombatReadinessState` from the existing `Character` (pure-cast → `ATacticalMovementCharacter` → Get Combat Readiness State) → **Select by state** to a target: **ADS = 0, MovementReady = 8, LowReady = 22, Sul = 38** → **`PosturePitch = FInterpTo(PosturePitch, target, DeltaTimeX, 8)`** → Set `PosturePitch`; Compile + Save.
- **THEN (Claude, after the manual step):** **verify in PIE that `PosturePitch` changes per readiness state (0/8/22/38)** BEFORE any AnimGraph work.
