...
**CURRENT WORK — Slice 2 (weapon posture visualization) — IN PROGRESS, PAUSED (2026-07-04).** Goal: make the four readiness states **visually distinct** — Sul / LowReady / MovementReady / ADS (today all four share the one baseline rifle-hold pose). Gate approved in docs `10`. **On branch `feature/weapon-posture-visualization`** (from `main` `acae9cb`). **Approved approach = Option B (ABP-only; readiness drives a separate `PosturePitch`, NOT `PitchN`/`AO_Rifle`, which stay purely for aiming).**

- **Implementation state (as of the 2026-07-04 UE crash — see below):**
  - ✅ `PosturePitch : float` variable added to `ABP_TacticalRifle`.
  - ✅ **EventGraph posture logic ADDED by the user in-editor and Compiled + Saved** — `Get Character → (pure) Cast to ATacticalMovementCharacter → Get CombatReadinessState → Select (ADS=0 / MovementReady=8 / LowReady=22 / Sul=38) → FInterpTo(PosturePitch, target, DeltaTimeX, 8) → Set PosturePitch`, with the Set node's exec appended after the existing `Set Pitch N` tail. **MCP-verified: ABP compiles clean; existing `SetVelocity`/`SetPitchN` intact; `PitchN`/`AO_Rifle` NOT modified.** Persisted to disk (`ABP_TacticalRifle.uasset` = 427 KB vs 411 KB on `main`, saved 19:58 before the crash).
  - ⏳ **`PosturePitch` PIE validation NOT completed** — UE **crashed** during the live-PIE read that was checking the 0/8/22/38 values (a *read* of the running game; no edit was in flight, assets not corrupted). Must be re-run after restart.
  - ❌ AnimGraph Modify Bone **NOT started** (do not start until PosturePitch PIE-validated).
- **⚠️ Incidental out-of-scope change to revert:** `SK_Mannequin.uasset` shows modified (an incidental editor/crash re-save — Slice 2 must not touch it). Revert with `git checkout -- Content/Characters/Mannequins/Meshes/SK_Mannequin.uasset` (when the editor is closed) so the only Slice-2 change stays `ABP_TacticalRifle.uasset`.
- **RESUME STEPS after UE restart:** (1) reopen project; (2) MCP-verify `ABP_TacticalRifle` still compiles + has the posture logic; (3) revert `SK_Mannequin`; (4) re-run the PIE `PosturePitch` check per readiness (gentler than the live-read that crashed — e.g. set readiness via the pawn and read the anim instance, or user presses 1/2/3/4 and Claude samples); (5) only then the AnimGraph Modify Bone step (`spine_03`, Component Space, Add to Existing, driven by `PosturePitch`, after the aim offset → Root).
- **Uncommitted:** `ABP_TacticalRifle.uasset` (the whole Slice-2 EventGraph work) is **saved on disk but NOT committed** — recommend committing it (with user approval) to fully secure it post-crash. Rollback if needed: `git checkout -- .../ABP_TacticalRifle.uasset` (restores `main`; `ABP_Unarmed` untouched).
- **AnimGraph Modify Bone step — NOT started, do NOT start until EventGraph is verified.** Spec (approved, for later): Transform (Modify) Bone on **`spine_03`** (confirmed to exist), Rotation Mode = Add to Existing, Space = Component Space, Pitch driven by `PosturePitch`, Translation/Scale ignored, Alpha 1.0, placed after the aim offset (`RotationOffsetBlendSpace_0`) → Root; invert sign once if positive pitch raises instead of lowers (no random tuning).
- **Expected changed implementation file remains ONLY** `Content/Characters/Mannequins/Anims/Rifle/ABP_TacticalRifle.uasset`. **Do NOT touch** C++, input, movement DataTables, readiness rules, `BP_ThirdPersonCharacter`, `ABP_Unarmed` (rollback), Gameplay Tags, GAS, firing/reload/ammo/recoil/pickup/drop/inventory.
- **Uncommitted WIP:** `ABP_TacticalRifle.uasset` (the `PosturePitch` var add) is intentionally **left uncommitted** on the branch — do not commit it without explicit user approval. **Rollback if needed:** `git checkout -- Content/Characters/Mannequins/Anims/Rifle/ABP_TacticalRifle.uasset` (restores the `main` version; `ABP_Unarmed` untouched).
