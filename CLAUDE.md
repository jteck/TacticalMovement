...
> **Verify live git state before acting** (`git status`, `git log --oneline --decorate`). Hashes below were verified on 2026-07-16 but re-check.

### 1. Current repo state (verified 2026-07-16)
...
- **Do not assume a `main` tip hash — always verify the live tip** (`git rev-parse origin/main`, `gh pr view`). **FP-8 feature merge = `64ebbe7`** (PR #16; FP-8 completion `bc55ebd` built on WIP scaffold checkpoint `7fe799c`). **FP-7 feature merge = `0b6efc4`** (PR #13; FP-7 checkpoint `15125ec` + cleanup `9b0514c`). **FP-7 and FP-8 are MERGED into `main`.** Mainline chronology: pre-FP `acae9cb` → **PR #7** merge `be92d7b` (FP-0…FP-4, first FP landing) → **PR #8** merge `8cd8d8b` (docs post-FP-4) → **PR #9** merge `cb1910c` (FP-5) → **PR #10** merge `871ee79` (docs post-FP-5) → **PR #11** merge `fa561b7` (FP-6) → **PR #13** merge `0b6efc4` (FP-7) → docs PR #14 `9f97d75` → docs reconciliation PR #15 `dc43134` → **PR #16** merge `64ebbe7` (FP-8). PRs #13/#16 are **MERGED, not pending**.
- **CURRENT BRANCH:** `main` — post FP-8; no FP feature work is in flight. (Feature branches kept intact post-merge, not deleted — incl. `feature/fp8-tactical-fp-ads-fov` @ `bc55ebd`, `feature/fp7-tactical-fp-weapon-reload-sync` @ `9b0514c`, `feature/fp6-first-person-view-weapon` @ `6d5cac1`, plus the `docs/fp7-complete` / `docs/fp8-complete` doc branches.)
...
- **Limitation as of FP-6 (historical — RESOLVED in FP-7):** at FP-6 the receiver/skeletal magazine were **mechanically static** during reload (arms animated; weapon did not). **FP-7 later added synchronized weapon reload montages (see 3d), so this is no longer current.** The magazine's remaining two-mag overlap is intentional (Infima design); deferred fidelity = magazine AnimBP/bullet meshes.
...
### 3d. FP-7 — COMPLETE, VERIFIED, MERGED to `main` (PR #13, merge `0b6efc4`)
FP-7 adds first-person **weapon** reload synchronization (arms + weapon reload montages in sync). Implemented on `feature/fp7-tactical-fp-weapon-reload-sync` (checkpoint `15125ec` + cleanup `9b0514c`), merged via **PR #13** (merge `0b6efc4`).
- **Exact six-file scope:** `ABP_TacticalFP_Weapon.uasset`, `Montages/AM_TacticalFP_WeaponReload.uasset`, `Notifies/ANS_TacticalFP_MagVisibility.uasset`, `InfimaGames/…/AssaultRifle/Animations/Weapon/FP/A_TFA_FP_WEP_AR_Reload.uasset` + `…_Aimed.uasset`, and `ThirdPerson/Blueprints/BP_ThirdPersonCharacter.uasset`.
- **Dual-slot weapon reload AnimBP/montage:** `ABP_TacticalFP_Weapon` blends `DefaultSlot`/`Aiming`; corrected `Blend Poses by bool` wiring = **Aiming→True Pose, DefaultSlot→False Pose, `bActiveValue`←`IsADS`** (`BlendListByBool`: bActiveValue TRUE → BlendPose_0).
- **Permanent, presentation-only `PlayFirstPersonReload` seam retained** as a Blueprint-callable function — **not yet connected to production reload gameplay** (its only prior caller, the temp trigger, was removed).
- **Reserve-mag visibility refinement + interruption restoration:** `ANS_TacticalFP_MagVisibility` window + canonical `RestoreMagVisibility` (invoked on `BeginPlay`).
- **Temporary R-key/Tick reload test trigger and all diagnostics removed** from `BP_ThirdPersonCharacter` (12 nodes + 1 orphan debris; `DiagReload` already gone).
- **Verification:** compile **0/0** (`BP_ThirdPersonCharacter`, `ABP_TacticalFP_Weapon`, `ANS_TacticalFP_MagVisibility`); **Readiness 12/12**; deps **no `BP_TFA`/demo leaks**; protected `ABP_TacticalFP` hash **unchanged** (`5d7f5cb`).
- **ADS "magazine split" investigation — resolved (presentation, not a bug):** the standard tactical reload **intentionally overlaps two magazines** (reserve + main; Infima's own design, replicated). The apparent ADS separation is **framing**: Infima's ADS blends camera FOV **100→78** (plus a positional aiming adjustment); **TacticalMovement held FOV 100 at FP-7 — now RESOLVED in FP-8, which adds the FOV 100→78 ADS blend (see 3e).** The magazine **depletion pose changes only internal follower/round geometry, not magazine body placement** (Persona frame-0-vs-30 diff: body pixel-identical). Placement chain is offset-free (`SnapToTarget`, identity mesh components).
- **Deferred / out of scope (as of FP-7):** ADS-framing (FOV 100→78) — **DONE in FP-8 (see 3e)**; secondary fidelity = porting the magazine AnimBP/bullet meshes (still deferred). **No firing, ammo, inventory, or gameplay reload input** implemented.

### 3e. FP-8 — COMPLETE, VERIFIED, MERGED to `main` (PR #16, merge `64ebbe7`)
FP-8 adds the **ADS field-of-view framing** identified by the FP-7 investigation: presentation-only FOV **100 → 78** on aim-down-sights, so the first-person weapon/iron-sights framing (and the intended two-magazine overlap) reads correctly instead of sitting wide. Implemented on `feature/fp8-tactical-fp-ads-fov` (WIP scaffold checkpoint `7fe799c` + completion `bc55ebd`), merged via **PR #16** (merge `64ebbe7`).
- **Exact scope = one file:** `Content/ThirdPerson/Blueprints/BP_ThirdPersonCharacter.uasset` (cumulative `main…feature` diff = this file only; 17 nodes added to the EventGraph). No other asset touched.
- **Behavior:** FOV **100** at Sul/LowReady/MovementReady, **78** in ADS. Driven by an **internal Timeline `TL_ADS_FOV`, length 0.36 s**, with a **source-shaped eased curve** (float track `Alpha`, keys ≈ (0.00,0.00),(0.15,0.14),(0.36,1.00), cubic/auto — asymmetric ease, not linear). **Edge-detected Play/Reverse** off the existing `ECombatReadinessState` (Event Tick compares `IsADS` vs `bWasADS`; on change → Play on ADS-enter / Reverse on ADS-exit). Timeline `Update` → `Lerp(100,78,Alpha)` → `SetFieldOfView` on `FirstPersonCamera`. **BeginPlay init** (appended after `HideBoneByName`) seeds `bWasADS`, FOV (78 if ADS else 100), and timeline position (`SetNewTime` 0.36 if ADS else 0.0) so a session that starts in ADS is framed correctly.
- **No positional ADS offset** — FOV-only (TM's authored ADS pose already aligns sights; positional pull-in stays a separately gated fallback, not implemented).
- **Verification (all passed):** compile **0/0**; **Readiness 12/12**; **no new demo/`BP_TFA` deps** (dependency set unchanged from base); protected `ABP_TacticalFP` hash **unchanged** (`5d7f5cb`). **Runtime checklist passed** (fresh Standalone `-game`, `showdebug camera`): hipfire FOV 100 / ADS FOV 78 on-screen; iron sights centered; horizon level; smooth mid-transition RMB-release reversal (no snap); rapid-toggle stability (no stuck FOV); key-4 dev latch → 78; keys 1/2/3 → 100; correct mouse pitch/yaw + WASD.
- **Reload-while-ADS: not exercised** — no production reload caller exists yet (FP-7 removed the temporary R-key caller); recorded as unavailable rather than reintroducing temporary reload wiring.
- **Wiring lesson (durable):** authored via MCP `BlueprintTools` **granular `create_node`/`connect_pins`/`set_pin_value`** — **NOT `write_graph_dsl`**, which regenerates the whole graph and would destroy the pre-authored Timeline keyframes. `get_node_type_pins` does not persist a node. Node-type gotchas: enum equality = `Utilities|Enum|Equal(Enum)` (B pin = `"ADS"`); variable nodes strip the `b` prefix (`Variables|Default|GetWasADS`/`SetWasADS`); `Class|CameraComponent|SetFieldOfView` spawns a property-set node with a `self` = Camera Component target pin; `Math|Float|SelectFloat(A,B,bPickA)` returns A when bPickA true; the promotable `!=` resolves to `Math|Boolean|NotEqual(Boolean)` once bools are connected.

### 3f. Native FP rendering + packaging/shader/AssetManager infra — VALIDATED
Infra fixes landed on `main` first, then the rendering candidate:
- **Packaging hygiene (PR #18, `c82c694`):** dev/MCP plugins (AIAssistant/ModelContextProtocol/AllToolsets) `TargetAllowList:["Editor"]` + a GameFeatureData AssetManager rule in DefaultGame.ini → cook no longer aborts with Error_UnknownCookFailure; those plugins + GameFeatures are absent from the shipped game; StateTree/GameplayStateTree retained.
- **Mac SM5+SM6 (PR #19, `bd12291`):** DefaultEngine.ini Mac TargetedRHIs cooks both SF_METAL_SM5 and SF_METAL_SM6. UE 5.8 requires the applicable macOS version and Apple8-class hardware for Metal SM6. This M1 Pro was observed selecting SM5 successfully; SM6 artifacts confirmed cooked, but runtime SM6 selection on M2+ hardware remains unverified.
- **GameFeatureData rule editor-only (PR #20, `d039233`):** DefaultGame.ini rule bIsEditorOnly=True → packaged runtime skips the scan (no handled ensure); Editor cook still scans it (cook still passes).
- **Native FP rendering (this PR):** FirstPersonScale 0.6 (FirstPersonCamera); 7 FP primitives → FirstPerson/bOnlyOwnerSee; CharacterMesh0/WeaponMesh → WorldSpaceRepresentation/bOwnerNoSee; DefaultEngine.ini NearClipPlane=5. At FirstPersonScale 0.6, NearClipPlane 5 eliminated the observed missing/separated weapon geometry; no visible modular separation remained in the tested configuration (NOT a general proof against every modular-morph issue). NearClipPlane=5 is project-wide → regression-test future maps, optics, particles/VFX, additional weapons, and long-distance depth precision as added. Temporary R-key PlayFirstPersonReload caller removed; permanent function retained with no caller (production reload input still out of scope — reload intentionally unavailable in-game).
- **FirstPersonScale is render-only** and does not move authoritative world-space gameplay transforms. **Deferred firing/obstruction:** before firing or weapon obstruction, validate the rendered muzzle against the raw/world muzzle socket and keep gameplay authority in world space.
- **Validation (precise):** compile 0/0; Readiness 12/12; ABP_TacticalFP 5d7f5cb / ABP_TacticalFP_Weapon f790d95 unchanged. Pre-cleanup combined package (with the R-key harness) passed hip reload, ADS reload, retrigger/interruption, magazine-state checks; final harness-free production package passed MovementReady/ADS, close-wall views, movement/look/jump, clean exit (reload unavailable). Previously passed separate-process Standalone two-client ownership/visibility test (NOT a packaged multiplayer test).
- **KNOWN separate unfixed gaps:** remote vertical aim pitch not represented; remote support-hand below the rifle fore-end.

### 4. NEXT ACTION (resume here)
**FP-0…FP-8 are complete, verified, and MERGED into `main`** (FP-0…FP-4 via PR #7 `be92d7b`; docs via PR #8 `8cd8d8b`; FP-5 via PR #9 `cb1910c`; docs via PR #10 `871ee79`; FP-6 via PR #11 `fa561b7`; **FP-7 via PR #13 `0b6efc4`**; **FP-8 via PR #16 `64ebbe7`**). **Packaging/shader/AssetManager infra also merged: packaging hygiene PR #18 (`c82c694`), Mac SM5+SM6 PR #19 (`bd12291`), GameFeatureData editor-only PR #20 (`d039233`).** **Native first-person rendering (FirstPersonScale 0.6 + NearClipPlane 5, harness-free) is validated and recorded in §3f; validation was packaged single-player on M1 Pro plus the previously-passed separate-process Standalone two-client ownership/visibility test, with remote aim-pitch and support-hand placement still unfixed.** Working tree is clean and **no FP work is in flight**. **Verify the live `main` tip** (`git rev-parse origin/main`, `gh pr view`) rather than assuming a hash — this doc's snapshot advances with each docs merge.
**No slice is currently approved.** Remaining separately-gated future work (each needs its own approval): (a) the **positional ADS offset** fallback (FP-8 was FOV-only by design); (b) wiring the permanent `PlayFirstPersonReload` seam to **production reload gameplay** (no reload caller exists yet); (c) secondary magazine fidelity (AnimBP/bullet meshes); (d) reduce reload camera shake. **Do not** begin any of these, or any firing/ammo/inventory/gameplay-reload work, without an approved slice. Same gated pattern: read-only preflight → propose scope → await explicit approval → implement on a new `feature/*` branch → verify → commit gate → push gate. Re-verify live git + PR state (`git`, `gh pr view`) first.
