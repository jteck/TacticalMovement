"""Dry-run-first finished-surface support correction for Salvage Yard hand tools."""
...
if len(targets) != EXPECTED_ACTORS:
    raise RuntimeError("SUNSCAR_HAND_TOOL_SUPPORT_SCOPE actor_count=%d" % len(targets))
...
if apply_requested and blockers:
    raise RuntimeError("SUNSCAR_HAND_TOOL_SUPPORT_BLOCKED blockers=%d" % len(blockers))
...
name = "old_town_hand_tool_support_fix_apply_v1.json" if apply_requested else "old_town_hand_tool_support_fix_dry_run_v1.json"
report = common.write_json_report(config, name, payload)
unreal.log("SUNSCAR_HAND_TOOL_SUPPORT mode=%s corrections=%d blockers=%d report=%s" % ("APPLY_UNSAVED" if apply_requested else "DRY_RUN", len(corrections), len(blockers), report))
print("SUNSCAR_HAND_TOOL_SUPPORT", len(corrections), len(blockers), report)
