"""Save only the verified Old Town rooftop-utility World Partition packages."""
...
if len(actors) != EXPECTED_ACTORS:
    raise RuntimeError("SUNSCAR_ROOFTOP_UTILITY_SAVE_REFUSED expected_actors=%d actual=%d" % (EXPECTED_ACTORS, len(actors)))

audit_path = os.path.join(common.report_directory(config), "old_town_rooftop_utility_audit_v1.json")
...
if audit.get("actor_count") != EXPECTED_ACTORS or audit.get("review_required_count") != 0 or audit.get("pair_overlap_count") != 0:
    raise RuntimeError("SUNSCAR_ROOFTOP_UTILITY_SAVE_REFUSED audit_not_clean")
...
if content:
    raise RuntimeError("SUNSCAR_ROOFTOP_UTILITY_SAVE_REFUSED dirty_content=%d" % len(content))
...
if unexpected or len(names) != EXPECTED_PACKAGES:
    raise RuntimeError(
        "SUNSCAR_ROOFTOP_UTILITY_SAVE_REFUSED packages=%d unexpected=%s"
        % (len(names), " | ".join(unexpected))
    )
if not unreal.EditorLoadingAndSavingUtils.save_packages(maps, True):
    raise RuntimeError("SUNSCAR_ROOFTOP_UTILITY_SAVE_FAILED")
...
if remaining:
    raise RuntimeError("SUNSCAR_ROOFTOP_UTILITY_SAVE_INCOMPLETE " + " | ".join(remaining))

payload = {
    "schema_version": 1,
    "status": "exact_rooftop_utility_saved",
    "context": context,
...
report = common.write_json_report(config, "old_town_save_rooftop_utility_v1.json", payload)
unreal.log("SUNSCAR_ROOFTOP_UTILITY_SAVE actors=%d packages=%d report=%s" % (len(actors), len(names), report))
print("SUNSCAR_ROOFTOP_UTILITY_SAVE", len(actors), len(names), report)
