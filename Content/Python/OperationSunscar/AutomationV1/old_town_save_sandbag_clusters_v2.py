"""Save exactly the sandbag cluster actor packages corrected by V2."""
...
FIX_TAG = "SunscarSandbagClusterCorrectionV2"
config = common.load_config()
context = common.require_safe_context(config, write_requested=False)
actors = [
    actor
    for actor in common.actor_subsystem().get_all_level_actors()
    if FIX_TAG in common.actor_tags(actor)
]
if len(actors) != 14:
    raise RuntimeError("SUNSCAR_SANDBAG_V2_SAVE_REFUSED actor_count=%d" % len(actors))
...
if dirty_content or unexpected or missing:
    raise RuntimeError(
        "SUNSCAR_SANDBAG_V2_SAVE_REFUSED content=%d unexpected=%s missing=%s"
...
if not unreal.EditorLoadingAndSavingUtils.save_packages(list(target_packages), True):
    raise RuntimeError("SUNSCAR_SANDBAG_V2_SAVE_FAILED")
...
report = common.write_json_report(config, "old_town_save_sandbag_clusters_v2.json", payload)
unreal.log("SUNSCAR_SANDBAG_V2_SAVE packages=%d remaining=%d report=%s" % (len(target_packages), len(remaining), report))
print("SUNSCAR_SANDBAG_V2_SAVE", len(target_packages), report)
