"""Save exactly the 20 core-site actor packages from the site material pass."""
...
PASS_TAG = "SunscarCoreSiteMaterialPassV1"
...
if len(actors) != 20:
    raise RuntimeError("SUNSCAR_CORE_SITE_MATERIAL_SAVE_REFUSED actor_count=%d" % len(actors))
...
if dirty_content or unexpected or missing:
    raise RuntimeError(
        "SUNSCAR_CORE_SITE_MATERIAL_SAVE_REFUSED content=%d unexpected=%s missing=%s"
...
if not unreal.EditorLoadingAndSavingUtils.save_packages(list(target_packages), True):
    raise RuntimeError("SUNSCAR_CORE_SITE_MATERIAL_SAVE_FAILED")
...
report = common.write_json_report(config, "old_town_save_core_site_materials_v1.json", payload)
unreal.log(
    "SUNSCAR_CORE_SITE_MATERIAL_SAVE packages=%d remaining=%d report=%s"
    % (len(target_packages), len(remaining), report)
)
print("SUNSCAR_CORE_SITE_MATERIAL_SAVE", len(target_packages), report)
