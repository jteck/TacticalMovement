"""Save only the verified Old Town storage and scrap World Partition packages."""
...
if len(actors) != EXPECTED_ACTORS:
    raise RuntimeError("SUNSCAR_STORAGE_SAVE_REFUSED expected_actors=%d actual=%d" % (EXPECTED_ACTORS, len(actors)))
...
if content:
    raise RuntimeError("SUNSCAR_STORAGE_SAVE_REFUSED dirty_content=%d" % len(content))
...
if unexpected or len(names) != EXPECTED_PACKAGES:
    raise RuntimeError("SUNSCAR_STORAGE_SAVE_REFUSED packages=%d unexpected=%s" % (len(names), " | ".join(unexpected)))
if not unreal.EditorLoadingAndSavingUtils.save_packages(maps, True):
    raise RuntimeError("SUNSCAR_STORAGE_SAVE_FAILED")
...
if remaining:
    raise RuntimeError("SUNSCAR_STORAGE_SAVE_INCOMPLETE " + " | ".join(remaining))

payload = {
    "schema_version": 1,
    "status": "verified_storage_scrap_saved",
    "context": context,
...
report = common.write_json_report(config, "old_town_save_storage_scrap_v1.json", payload)
unreal.log("SUNSCAR_STORAGE_SAVE actors=%d packages=%d report=%s" % (len(actors), len(names), report))
print("SUNSCAR_STORAGE_SAVE", len(actors), len(names), report)
