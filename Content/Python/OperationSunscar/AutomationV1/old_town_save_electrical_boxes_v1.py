"""Save only the five verified packages for three Old Town electrical boxes."""
...
if len(actors) != 3:
    raise RuntimeError("SUNSCAR_ELECTRICAL_SAVE_REFUSED actors=%d" % len(actors))
...
if content:
    raise RuntimeError("SUNSCAR_ELECTRICAL_SAVE_REFUSED dirty_content=%d" % len(content))
...
if unexpected or len(names) != 5:
    raise RuntimeError("SUNSCAR_ELECTRICAL_SAVE_REFUSED packages=%d unexpected=%s" % (len(names), " | ".join(unexpected)))
if not unreal.EditorLoadingAndSavingUtils.save_packages(maps, True):
    raise RuntimeError("SUNSCAR_ELECTRICAL_SAVE_FAILED")
...
if remaining:
    raise RuntimeError("SUNSCAR_ELECTRICAL_SAVE_INCOMPLETE " + " | ".join(remaining))
payload = {
    "schema_version": 1,
    "status": "verified_electrical_boxes_saved",
    "context": context,
...
report = common.write_json_report(config, "old_town_save_electrical_boxes_v1.json", payload)
unreal.log("SUNSCAR_ELECTRICAL_SAVE actors=%d packages=%d report=%s" % (len(actors), len(names), report))
print("SUNSCAR_ELECTRICAL_SAVE", len(actors), len(names), report)
