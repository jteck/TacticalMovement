"""Save exactly the 50 reviewed SS_004/007/012 stucco facade packages."""
...
PASS_TAG = "SunscarFacadeExpansionV1"
...
if len(actors) != 50:
    raise RuntimeError("SUNSCAR_STUCCO_EXPANSION_SAVE_REFUSED actors=%d" % len(actors))
for actor in actors:
    ...
    if material is None or material.get_path_name() != TARGET_PATH:
        raise RuntimeError("SUNSCAR_STUCCO_EXPANSION_SAVE_REFUSED material=" + actor.get_actor_label())
...
if dirty_content or dirty_maps != target_names:
    raise RuntimeError(
        "SUNSCAR_STUCCO_EXPANSION_SAVE_REFUSED content=%s maps=%s"
...
if not unreal.EditorLoadingAndSavingUtils.save_packages(list(packages), True):
    raise RuntimeError("SUNSCAR_STUCCO_EXPANSION_SAVE_FAILED")
remaining = sorted(
    package.get_name()
    for package in (
...
if remaining:
    raise RuntimeError("SUNSCAR_STUCCO_EXPANSION_SAVE_DIRTY_AFTER %s" % "|".join(remaining))
payload = {
    "schema_version": 1,
    "status": "exact_stucco_facade_packages_saved",
    "context": context,
...
report = common.write_json_report(config, "old_town_save_stucco_facades_v1.json", payload)
unreal.log("SUNSCAR_STUCCO_EXPANSION_SAVE packages=%d report=%s" % (len(packages), report))
print("SUNSCAR_STUCCO_EXPANSION_SAVE", len(packages), report)
