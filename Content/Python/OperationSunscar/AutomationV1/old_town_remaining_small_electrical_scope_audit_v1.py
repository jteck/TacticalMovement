"""Read-only ownership audit for dirty packages from the 13-box utility pass."""
...
TAG = "SunscarOldTownElectricalBoxesV2"
EXPECTED_ACTORS = 13
EXPECTED_FOLDERS = 7
...
external_objects_prefix = "/Game/__ExternalObjects__/Maps/Blockout/Lvl_Blockout_01/"
folders_valid = all(
    record["package"].startswith(external_objects_prefix)
    and len(record["asset_classes"]) == 1
    and "ActorFolder" in record["asset_classes"][0]
    for record in extra_records
)
safe_scope = (
    len(targets) == EXPECTED_ACTORS
    and not dirty_content
    and len(target_packages) == EXPECTED_ACTORS
    and len(extra_records) == EXPECTED_FOLDERS
    and folders_valid
    and len(dirty_maps) == EXPECTED_ACTORS + EXPECTED_FOLDERS
)
payload = {
    "schema_version": 1,
...
report = common.write_json_report(config, "old_town_remaining_small_electrical_scope_audit_v1.json", payload)
unreal.log(
    "SUNSCAR_REMAINING_ELECTRICAL_SCOPE actors=%d dirty=%d folders=%d safe=%s report=%s"
    % (len(targets), len(dirty_maps), len(extra_records), safe_scope, report)
)
print("SUNSCAR_REMAINING_ELECTRICAL_SCOPE", len(targets), len(dirty_maps), len(extra_records), safe_scope, report)
