"""Temporarily hide Old Town TextRender navigation labels without dirtying packages."""
...
if before_content != after_content or before_maps != after_maps:
    raise RuntimeError("SUNSCAR_HIDE_LABELS_DIRTIED_PACKAGES")
payload = {
    "schema_version": 1,
    "status": "temporary_labels_hidden",
    "context": context,
...
report = common.write_json_report(config, "old_town_hide_temporary_labels_v1.json", payload)
unreal.log("SUNSCAR_HIDE_LABELS actors=%d report=%s" % (len(targets), report))
print("SUNSCAR_HIDE_LABELS", len(targets), report)
