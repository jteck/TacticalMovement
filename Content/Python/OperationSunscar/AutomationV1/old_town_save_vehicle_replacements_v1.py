"""Save exactly the five existing actor packages used for vehicle replacement."""
...
if len(actors) != 5:
    raise RuntimeError("SUNSCAR_VEHICLE_REPLACE_SAVE_REFUSED actor_count=%d" % len(actors))
...
if dirty_content or unexpected or missing:
    raise RuntimeError(
        "SUNSCAR_VEHICLE_REPLACE_SAVE_REFUSED content=%d unexpected=%s missing=%s"
...
if not unreal.EditorLoadingAndSavingUtils.save_packages(list(target_packages), True):
    raise RuntimeError("SUNSCAR_VEHICLE_REPLACE_SAVE_FAILED")
remaining = sorted(
    package.get_name()
...
report = common.write_json_report(config, "old_town_save_vehicle_replacements_v1.json", payload)
unreal.log("SUNSCAR_VEHICLE_REPLACE_SAVE packages=%d remaining=%d report=%s" % (len(target_packages), len(remaining), report))
print("SUNSCAR_VEHICLE_REPLACE_SAVE", len(target_packages), report)
