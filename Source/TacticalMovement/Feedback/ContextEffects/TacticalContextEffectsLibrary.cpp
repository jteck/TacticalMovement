...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TacticalContextEffectsLibrary)


void UTacticalContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UTacticalContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UTacticalContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UTacticalContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FTacticalContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UTacticalActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FTacticalContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UTacticalActiveContextEffects* NewActiveContextEffects = NewObject<UTacticalActiveContextEffects>(this);
	...
	this->TacticalContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UTacticalContextEffectsLibrary::TacticalContextEffectLibraryLoadingComplete(
	TArray<UTacticalActiveContextEffects*> TacticalActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(TacticalActiveContextEffects);
}
