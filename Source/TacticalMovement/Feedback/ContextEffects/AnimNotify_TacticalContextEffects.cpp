...
#include "AnimNotify_TacticalContextEffects.h"
#include "Feedback/ContextEffects/TacticalContextEffectsLibrary.h"
#include "TacticalContextEffectsInterface.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_TacticalContextEffects)



UAnimNotify_TacticalContextEffects::UAnimNotify_TacticalContextEffects()
{
}

void UAnimNotify_TacticalContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_TacticalContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_TacticalContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_TacticalContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> TacticalContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UTacticalContextEffectsInterface>())
			{
				// If so, add it to the Array
				TacticalContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UTacticalContextEffectsInterface>())
					{
						TacticalContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* TacticalContextEffectImplementingObject : TacticalContextEffectImplementingObjects)
			{
				if (TacticalContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					ITacticalContextEffectsInterface::Execute_AnimMotionEffect(TacticalContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UTacticalContextEffectLibrary type
						if (UTacticalContextEffectsLibrary* EffectLibrary = Cast<UTacticalContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_TacticalContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_TacticalContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FTacticalContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FTacticalContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FTacticalContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
